...
//! moves into the deleted slot. `IdMapIndex32` wraps the positional index
//! with a bidirectional `id ↔ slot` mapping so callers can identify
//! vectors by a stable `u32` ID that doesn't change when other vectors
//! are inserted or removed.
//!
//! A bidirectional hash-table-backed `u32 ↔ slot` mapping layered over
...
//!
//! let mut index = IdMapIndex32::new(1536, 4).unwrap();
...
impl IdMapIndex32 {
    ...
    pub fn add_with_ids(&mut self, vectors: &[f32], ids: &[u32]) -> Result<(), AddError> {
        let dim = self.inner.dim_opt().expect(
            "IdMapIndex32 dim is not set; use add_with_ids_2d(vectors, dim, ids) \
             on the first add or construct with IdMapIndex32::new(dim, bit_width)",
        );
    ...
    pub fn add_with_ids_2d(
        ...
        // this call.
        let mut seen_this_call: std::collections::HashSet<u32> =
            std::collections::HashSet::with_capacity(n);
        for &id in ids {
            if self.id_to_slot.contains_key(&id) || !seen_this_call.insert(id) {
                return Err(AddError::IdAlreadyPresent(id as u64));
            }
        }
    ...
    /// is `queries.len() / dim`.
    pub fn search(&self, queries: &[f32], k: usize) -> (Vec<f32>, Vec<u32>) {
    ...
    pub fn load(path: impl AsRef<Path>) -> std::io::Result<Self> {
        ...
        let id_to_slot: HashMap<u32, usize> = slot_to_id
            .iter()
            .enumerate()
            .map(|(slot, &id)| (id, slot))
            .collect();
